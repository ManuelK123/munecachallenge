...
function iniciarNivel7EscaleraMapa() {
    ...
    function comprobarColision(nx, ny) {
        // 1. Límite superior estricto del pasto (evita que suba muy arriba fuera de la franja verde)
        ...
        // Si está en la zona del puente (en X de 310 a 380), evaluamos el carril Y del puente (ej. entre 450 y 520)
        let enZonaPuenteX = (nx + jugador.ancho > 310 && nx < 380);
        if (enZonaPuenteX) {
            let medioPuenteY = ny + jugador.alto / 2;
            // Si está fuera del medio del puente (muy arriba o muy abajo de este), colisiona con las barandas
    ...
    function actualizar() {
        ...
        // Detectar si llega al extremo derecho superior (cerca de los árboles superiores / puente) para avanzar al siguiente escenario
        // Zona del cuadro detector derecho superior: X > 365 y Y cerca de los árboles superiores
        ...
        if (jugador.x <= 25 && indiceEscenarioActual > 0) {
            indiceEscenarioActual--;
            jugador.x = 330; // Aparece en el lado derecho del escenario anterior
            stageNameTxt.innerText = listaEscenarios[indiceEscenarioActual].split('.')[0];
        }
    ...
    function renderizar() {
        ...
        // 1. Dibujar la imagen del escenario actual (pioa, piob, pioc, pioz)
        ...
        // 3. Aplicar efecto de oscurecimiento (Fade) durante las transiciones de 4 segundos
    ...
    // ==========================================
    // CONTROLES TÁCTILES Y DE TECLADO (DIRECCIONALES LIBRES EN 2D)
    // ==========================================
